...
import { buildInitialOptimaDraft } from './optima.mock'
import {
  appendOptimaRichHistory,
  readOptimaDisplayedHistory,
  readOptimaSuggestionHistory,
...
const initialOptimaDraft = buildInitialOptimaDraft()

const draftStorageKey = `ordering-app:draft:${optimaVendor.id}`
const draftTimestampKey = `ordering-app:draft-ts:${optimaVendor.id}`
...
function buildOptimaEmptyTemplate(
  catalog: VendorItem[],
...
function readStoredDraft(): OrderDraft {
  try {
    ...
    if (!raw) return finalizeDraftWithBaseline(initialOptimaDraft)
    ...
    if (
      ...
      return finalizeDraftWithBaseline(initialOptimaDraft)
    }
    ...
    if (typeof parsed.internalNotes === 'string') {
      ...
      internalNotes = initialOptimaDraft.internalNotes
    }
    ...
    const deliveryDate =
      typeof parsed.deliveryDate === 'string'
        ? parsed.deliveryDate
        : initialOptimaDraft.deliveryDate

    let repFirstName =
      typeof parsed.repFirstName === 'string'
        ? parsed.repFirstName.trim()
        : ''
    if (!repFirstName || repFirstName !== optimaVendor.primaryRepFirstName) {
      repFirstName = initialOptimaDraft.repFirstName
    }
...
export function OptimaOrderSheet({ embedded, onSent }: Props) {
  ...
  const [optimaHistoryStamp, setOptimaHistoryStamp] = useState(0)

  const scheduleValidation = useMemo(
    () =>
      validateVendorDeliveryDate(optimaSchedulingRules, draft.deliveryDate),
    [draft.deliveryDate],
  )

  const disableOutboundActions =
    !scheduleValidation.isValid &&
    optimaSchedulingRules.invalidDateStrategy !== 'allow_blank_order'

  const lockChecklist =
    !scheduleValidation.isValid &&
    optimaSchedulingRules.invalidDateStrategy === 'block_order'
  ...
  const optimaHistoryForSort = useMemo((): VendorHistoryOrder[] => {
    return readOptimaDisplayedHistory(optimaCatalogItems).map((e) => ({
      date: e.orderDate,
  ...
  const checklistOrderedIds = useMemo(
    () =>
      orderedCatalogIdsForChecklist(
        optimaCatalogItems,
        optimaHistoryForSort,
        checklistSortMode,
      ),
    [checklistSortMode, optimaHistoryForSort],
  )
  ...
  const previewText = useMemo(
    () => buildOrderMessage(optimaVendor, optimaCatalogItems, draft),
    [draft],
  )
  ...
  const loadSuggestedOrder = () => {
    ...
    bumpDraftOnEdit((d) => ({
      ...d,
      items: generateOptimaSuggestedOrder(
        suggestionHistory,
        optimaCatalogItems,
      ),
    }))
  ...
  const clearAllItems = () => {
    bumpDraftOnEdit((d) => ({
      ...d,
      items: buildOptimaEmptyTemplate(optimaCatalogItems, d.items),
    }))
  }
  ...
  const logNativeSendExecution = () => {
    const method = resolveVendorPlatformConfig(optimaPlatformConfig).settings
      .orderPlacement.method
    appendVendorExecutionEvent({
      vendorId: optimaVendor.id,
      placedAt: Date.now(),
      deliveryDate: draft.deliveryDate,
      method,
  ...
  return (
    <div className={shellClass}>
          ...
          <VendorDeliveryDateBanner
            ...
            invalidDateStrategy={optimaSchedulingRules.invalidDateStrategy}
          ...
          <FinalizeOrderModal
            ...
            vendorName={optimaVendor.name}
            deliveryDate={draft.deliveryDate}
            itemCount={includedItemCount}
            previewText={previewText}
            placementMethod={optimaPlacement.method}
            destination={optimaPlacement.destination}
