...
import { aceEndicoSeedHistory } from './aceEndicoSeedHistory'
...
const HISTORY_KEY = 'ordering-app:orderHistoryRich:ace-endico'
const MAX_ENTRIES = 30
const VENDOR_ID = 'ace-endico'

export type AceEndicoRichHistoryEntry = VendorRichHistoryEntry

function seedRichEntries(catalog: VendorItem[]): VendorRichHistoryEntry[] {
  return aceEndicoSeedHistory.map((o) => ({
...
export function readAceEndicoDisplayedHistory(
  catalog: VendorItem[],
): VendorRichHistoryEntry[] {
...
export function readAceEndicoSuggestionHistory(
  catalog: VendorItem[],
): VendorHistoryOrder[] {
...
export function readAceEndicoSuggestionInspectorRows(
  catalog: VendorItem[],
...
export function toggleAceEndicoSuggestionRowExcluded(rowId: string): void {
  toggleExcludedSuggestionHistoryRow(VENDOR_ID, rowId)
}

export function deleteMostRecentAceEndicoAppSuggestionRow(): void {
...
export function appendAceEndicoRichHistory(
  draft: OrderDraft,
